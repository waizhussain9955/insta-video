...
export default function BulkVideoDownloader() {
  ...
  const handleFetch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Failed to fetch profile videos/reels. Please verify the account is public.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBulkDownload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedPosts.length; i++) {
        ...
        const filename = `${targetUsername}_video_${i + 1}.mp4`;
      ...
      link.download = `${targetUsername}_bulk_videos_${Date.now()}.zip`;
    ...
    <div className="mx-auto max-w-6xl px-4 py-16">
      ...
      <form onSubmit={handleFetch} className="glass-panel p-6 rounded-2xl mb-8 grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
        ...
        <div className="sm:col-span-3">
          ...
          >
            ...
            <span>{loading ? "Harvesting Reels & Videos..." : "Fetch Reels & Videos"}</span>
          </button>
        </div>
      </form>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-zinc-400 text-sm">Harvesting MP4 video links from profile reels...</p>
        </div>
      )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 pb-6 border-b border-white/10">
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <span>Select All Videos ({posts.length})</span>
              </button>
              <button
                onClick={deselectAll}
                className="flex items-center space-x-1.5 text-xs text-zinc-300 hover:text-white px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-lg transition"
